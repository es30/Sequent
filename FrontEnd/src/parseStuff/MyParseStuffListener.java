...
public class MyParseStuffListener implements ParseStuffListener {
	...
	static scala.Function3<Formula, Formula, scala.Option<error.Error>, Formula> formulaBiconditional = parseStuff.formulaBuilder.package$.MODULE$.formulaBiconditional();
	static scala.Function3<Formula, Formula, scala.Option<error.Error>, Formula> formulaImplication = parseStuff.formulaBuilder.package$.MODULE$.formulaImplication();
	static scala.Function3<Formula, Formula, scala.Option<error.Error>, Formula> formulaDisjunction = parseStuff.formulaBuilder.package$.MODULE$.formulaDisjunction();
	static scala.Function3<Formula, Formula, scala.Option<error.Error>, Formula> formulaConjunction = parseStuff.formulaBuilder.package$.MODULE$.formulaConjunction();
	static scala.Function2<P_Proposition, scala.Option<error.Error>, Formula> formulaProposition = parseStuff.formulaBuilder.package$.MODULE$.formulaProposition();
	...
	{
		sf = formulaAxiomatization.apply(ctx.a, P_Formula$.MODULE$.noError());
	}
	...
	@Override public void exitFormula(ParseStuffParser.FormulaContext ctx)
	{
		...
		if (fc != null && fc.f != null)
			f = formulaBiconditional.apply(f, fc.f, P_Formula$.MODULE$.noError());
		ctx.f = f;
	...
	@Override public void exitImplication(ParseStuffParser.ImplicationContext ctx)
	{
		...
		ParseStuffParser.FormulaContext fc = ctx.formula();
		if (fc != null && fc.f != null)
			f = formulaImplication.apply(f, fc.f, P_Formula$.MODULE$.noError());
		ctx.f = f;
	...
	@Override public void exitDisjunction(ParseStuffParser.DisjunctionContext ctx)
	{
		...
		while (! list.isEmpty())
		{
			f = formulaDisjunction.apply(f, list.get(0).f, P_Formula$.MODULE$.noError());
			list.remove(0);
		}
		ctx.f = f;
	...
	@Override public void exitConjunction(ParseStuffParser.ConjunctionContext ctx)
	{
		...
		while (! list.isEmpty())
		{
			f = formulaConjunction.apply(f, list.get(0).f, P_Formula$.MODULE$.noError());
			list.remove(0);
		}
		ctx.f = f;
	...
	{
		f = formulaProposition.apply(ctx.p, P_Formula$.MODULE$.noError());
		ctx.f = f;
	...
	{
        P_Predicate p = P_Formula$.MODULE$.lookupPredicate(ctx.Identifier().getText());
        f = formulaApplication.apply(p, ctx.applicand().a, P_Formula$.MODULE$.noError());
        ctx.f = f;
	...
	{
		f = ctx.atom().f;
		f = formulaNegation.apply(f, P_Formula$.MODULE$.noError());
		ctx.f = f;
	...
	@Override public void exitRelation(ParseStuffParser.RelationContext ctx)
	{
		...
		while (! list.isEmpty())
		{
			...
			f = formulaApplication.apply(ctx.p, termArray, P_Formula$.MODULE$.noError());
			list.remove(0);
		}
		ctx.f = f;
	...
	{
		f = formulaQuantification.apply(ctx.quantified_variable().q, ctx.quantified_variable().variable().v, ctx.atom().f, P_Formula$.MODULE$.noError());
		ctx.f = f;
	}
	...
	{
		f = formulaQuantification.apply(ctx.quantified_variable().q, ctx.quantified_variable().variable().v, ctx.quantified_formula().f, P_Formula$.MODULE$.noError());
		ctx.f = f;
	}
	...
	@Override public void exitTerm(ParseStuffParser.TermContext ctx)
	{
		...
		while (! list.isEmpty())
		{
			termArray[0] = t;
            termArray[1] = list.get(0).t;
            t = termApplication.apply(f, termArray, P_Formula$.MODULE$.noError());
			list.remove(0);
		}
		ctx.t = t;
		// System.out.print(" Term [" + f.toString() + "])");
	...
	@Override public void exitParticle_type_zero(ParseStuffParser.Particle_type_zeroContext ctx)
	{
		...
		t = new P_TermConstant(zero, P_Formula$.MODULE$.noError());
		ctx.t = t;
	...
	{
		ctx.t = termVariable.apply(ctx.variable().v, P_Formula$.MODULE$.noError());
		t = ctx.t;
	...
	{
		P_Function f = P_Term$.MODULE$.lookupFunction(ctx.Identifier().getText());
		t = termApplication.apply(f, ctx.applicand().a, P_Formula$.MODULE$.noError());
		ctx.t = t;
	}
	...
	{
		ctx.v = P_Term$.MODULE$.lookupVarName(ctx.Identifier().getText());
